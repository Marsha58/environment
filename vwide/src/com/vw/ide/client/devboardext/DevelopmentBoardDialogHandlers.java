...
public class DevelopmentBoardDialogHandlers {
	public static class DeletedProjectDialogHideHandler implements DialogHideHandler {
		...
		public void onDialogHide(DialogHideEvent event) {
			...
			if (s.equalsIgnoreCase("YES")) {
				...
				RemoteBrowserServiceBroker.requestForDeletingProject(FlowController.getLoggedAsUser(), owner.getSelectedItemInTheProjectTree()
						.getAbsolutePath(), owner.getSelectedItemInTheProjectTree().getProjectId(), new AnyDirOperationResultCallback(owner));
	...
	public static class DeleteFileDialogHideHandler implements DialogHideHandler {
		...
		public void onDialogHide(DialogHideEvent event) {
			...
			if (s.equalsIgnoreCase("YES") && owner.getSelectedItemInTheProjectTree() != null) {
				...
				RemoteBrowserServiceBroker.requestForFileDeleting(FlowController.getLoggedAsUser(), owner.getSelectedItemInTheProjectTree()
						.getAbsolutePath(), owner.getSelectedItemInTheProjectTree().getFileId(), new AnyFileOperationResultCallback(owner, true));
			}
		}
	}
		...
		public void onDialogHide(DialogHideEvent event) {
			if (box.getValue() != null) {
				if (owner.isValidFileName(box.getValue())) {
					RemoteBrowserServiceBroker.requestForFileCreating(FlowController.getLoggedAsUser(), Utils.extractJustPath(owner
							.getSelectedItemInTheProjectTree().getAbsolutePath()), box.getValue(), owner.getSelectedItemInTheProjectTree()
							.getProjectId(), 1000L, "", new AnyDirOperationResultCallback(owner));
				}
			}
		}
	}
	...
	public static class CreateFolderDialogHideHandler implements DialogHideHandler {
		...
		public void onDialogHide(DialogHideEvent event) {
			if (box.getValue() != null) {
				if (owner.isValidFileName(box.getValue())) {
					RemoteBrowserServiceBroker.requestForFolderCreating(FlowController.getLoggedAsUser(),
							Utils.extractJustPath(owner.getSelectedItemInTheProjectTree().getAbsolutePath()), box.getValue(),
		...
		private static class SaveFileDialogHideHandler implements DialogHideHandler {
			...
			public void onDialogHide(DialogHideEvent event) {
				if (event.getHideButton().name().equalsIgnoreCase("YES")) {
					RemoteBrowserServiceBroker.requestForFileSaving(FlowController.getLoggedAsUser(), (String) box.getData("fullFileName"),
							(Long) box.getData("projectId"), (Long) box.getData("fileId"), (String) box.getData("content"),
							new AnyFileOperationResultCallback(owner, true));
					RemoteBrowserServiceBroker.requestForFileRenaming(FlowController.getLoggedAsUser(), owner.getSelectedItemInTheProjectTree()
							.getAbsolutePath(), owner.getSelectedItemInTheProjectTree().getFileId(), fullNewFileName,
		...
		public void onDialogHide(DialogHideEvent event) {
			if (event.getHideButton().name().equalsIgnoreCase("ok")) {
				if (box.getValue() != null && owner.getSelectedItemInTheProjectTree() != null) {
					...
					String fullNewFileName = justFilePath + Utils.FILE_SEPARATOR + box.getValue();
					...
					if (currFileSheet.getIsFileEdited()) {
						...
						saveConfirmBox.addDialogHideHandler(new RenameFileDialogHideHandler.SaveFileDialogHideHandler(saveConfirmBox, owner,
								fullNewFileName));
